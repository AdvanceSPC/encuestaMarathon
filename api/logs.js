...
// Función para verificar si el contacto ya tiene encuesta programada hoy (en nuestra BD)
async function contactoYaTieneEncuestaHoy(conn, contactId, fechaControl) {
  try {
    const [rows] = await conn.execute(
      ...
      [contactId, fechaControl]
    );
    
    return rows[0].total > 0;
  } catch (error) {
    console.error(`Error verificando encuesta previa del contacto ${contactId}:`, error.message);
    return false;
  }
}
  ...
  for (const evento of eventos) {
    ...
    try {
      ...
      const [existingRows] = await conn.execute(
        `SELECT id, concepto, enviar_encuesta, fecha_creacion, contacto_id FROM registros WHERE id = ?`,
        [objectId]
      );
      ...
      if (existingRows.length > 0) {
        ...
        resultados.push({ 
          ...
          contacto_id: existingRows[0].contacto_id,
      ...
      // VALIDACIÓN : Verificar en nuestra BD si el contacto ya tiene encuesta programada hoy
      const yaTieneEncuestaHoy = await contactoYaTieneEncuestaHoy(conn, contactId, fechaControl);
      
      if (yaTieneEncuestaHoy) {
        console.log(`Contacto ${contactId} ya tiene encuesta programada para hoy (${fechaControl}). Negocio ${objectId} marcado como NO enviar.`);
        
        // Registrar en BD como NO enviar
        await conn.execute(
          `INSERT INTO registros (id, concepto, enviar_encuesta, fecha_creacion, fecha_cierre, contacto_id, razon_no_envio) 
           VALUES (?, ?, 0, NOW(), ?, ?, 'contacto_ya_tiene_encuesta_programada_hoy')`,
        ...
        resultados.push({ 
          ...
          razon: 'contacto_ya_tiene_encuesta_programada_hoy',
          fechaControl 
        });
      ...
      // Validación del límite del concepto
      const [rows] = await conn.execute(
        `SELECT COUNT(*) as total FROM registros 
      ...
      const razonNoEnvio = enviarEncuesta ? null : 'limite_concepto_alcanzado';

      try {
        await conn.execute(
          `INSERT INTO registros (id, concepto, enviar_encuesta, fecha_creacion, fecha_cierre, contacto_id, razon_no_envio) 
           VALUES (?, ?, ?, NOW(), ?, ?, ?)`,
          [objectId, concepto, enviarEncuestaFlag, fechaCierre, contactId, razonNoEnvio]
        );
        ...
        console.log(`Negocio ${objectId} (Contacto: ${contactId}) actualizado → ${enviarEncuesta ? 'SI' : 'NO'}${razonNoEnvio ? ` (${razonNoEnvio})` : ''}`);
        resultados.push({ 
          objectId, 
          concepto, 
          contacto_id: contactId,
